			...
			<tbody>
					@foreach($elementfilters as $elementfilter)
					    <tr class='row_'>
							...
							<td> - {{ $elementfilter->name }}</td>  
							<td class="option">
								<form action="{{ route('elementfilter.destroy',$elementfilter->id) }}" method="POST" accept-charset="utf-8">
									{!! csrf_field() !!}
									{!! method_field('DELETE') !!}
									<!-- <a href="" title="Xem chi tiết sản phẩm" class="tipS button greenB">
										<img src="{{ asset('_asset/images/icons/color/view.png') }}" />
									</a> -->
									<!-- <a href="" title="Thêm" class="tipS button greenB">
										<img src="{{ asset('_asset/images/icons/control/16/add.png') }}" />
									</a> -->
									<a href="{{ route('elementfilter.edit',$elementfilter->id) }}" title="Chỉnh sửa" class="tipS button blueB">
